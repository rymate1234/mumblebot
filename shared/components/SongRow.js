...
const SongLink = ({ item }) => {
  const isLink =
    item.website || (item.originalname && item.originalname.startsWith('http'))
  const Component = isLink ? Link : 'strong'

  return (
    <Component target="_blank" title href={item.website || item.originalname}>
      {item.title}
    </Component>
  )
}
...
const SongRow = ({ item, preview, setPreview }) => (
  <Station
    highlighted={preview.title === item.title}
    columns="2fr auto"
    rows={2}
    gap="2px"
  >
